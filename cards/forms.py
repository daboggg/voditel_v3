...
class DepartureAddForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # if cd.get('mileage_end') and not cd.get('distance'):
        #     departures_before = list(filter(
        #         lambda a: datetime.combine(a.date, a.return_time) < datetime.combine(cd.get('date'), cd.get('return_time')),
        #         departures))
        #     total = 0
        #     if departures_before:
        #         for departure in departures_before:
        #             total += departure.distance
        #     # print(departures_before, 'WEEEEEEEEEEEEEEEEEEEER')
        #     # print(total, 'REEEEEEEEEEEEEEEEEEEEEW')
        #     cd['distance'] = cd.get('mileage_end') - self.initial['card'].mileage + total
